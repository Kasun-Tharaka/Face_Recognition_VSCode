...
class TrainingDataCollector:
    ...
    def collectImagesFromCamera(self):
        ...
        # set a array with 4 zeros, bcz bbox output has array with 4 values
        max_bbox = np.zeros(4)

        if not (os.path.exists(self.args["output"])):
            os.makedirs(self.args["output"])

        # check whether there is a face or not
        while faces < max_faces:
            ret, frame = cap.read()
            frames += 1

            # set name for the saving image
            dtString = str(datetime.now().microsecond)
            # Get all faces on current frame with calling mtcnn
            bboxes = self.detector.detect_faces(frame)

            # check face is captured or not
            if len(bboxes) != 0:
